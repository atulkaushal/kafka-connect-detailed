...
curl -s 127.0.0.1:8083/connectors/file-stream-distributed/status | jq
# 6) Pause / Resume a Connector (no response if the call is succesful)
curl -s -X PUT 127.0.0.1:8083/connectors/file-stream-distributed/pause
curl -s -X PUT 127.0.0.1:8083/connectors/file-stream-distributed/resume
# 7) Get Connector Configuration
curl -s 127.0.0.1:8083/connectors/file-stream-distributed | jq
# 8) Delete our Connector
curl -s -X DELETE 127.0.0.1:8083/connectors/file-stream-distributed
# 9) Create a new Connector
curl -s -X POST -H "Content-Type: application/json" --data '{"name": "file-stream-distributed", "config":{"connector.class":"org.apache.kafka.connect.file.FileStreamSourceConnector","key.converter.schemas.enable":"true","file":"fileStream-file.txt","tasks.max":"1","value.converter.schemas.enable":"true","name":"file-stream-distributed","topic":"filestream-2-distributed","value.converter":"org.apache.kafka.connect.json.JsonConverter","key.converter":"org.apache.kafka.connect.json.JsonConverter"}}' http://127.0.0.1:8083/connectors | jq
# 10) Update Connector configuration
curl -s -X PUT -H "Content-Type: application/json" --data '{"connector.class":"org.apache.kafka.connect.file.FileStreamSourceConnector","key.converter.schemas.enable":"true","file":"fileStream-file.txt","tasks.max":"2","value.converter.schemas.enable":"true","name":"file-stream-distributed","topic":"filestream-2-distributed","value.converter":"org.apache.kafka.connect.json.JsonConverter","key.converter":"org.apache.kafka.connect.json.JsonConverter"}' 127.0.0.1:8083/connectors/file-stream-distributed/config | jq
