...
# Linux / Mac
docker run --rm -it -v "$(pwd)":/kakfaConnect --net=host landoop/fast-data-dev bash
# Windows Command Line:
docker run --rm -it -v %cd%:/kakfaConnect --net=host landoop/fast-data-dev bash
# Windows Powershell:
docker run --rm -it -v ${PWD}:/kakfaConnect --net=host landoop/fast-data-dev bash

# we launch the kafka connector in standalone mode:
cd /kafka-connect/source/filestream
# create the topic we write to with 3 partitions
kafka-topics --create --topic filestream-1-standalone --partitions 3 --replication-factor 1 --zookeeper 127.0.0.1:2181
...
kafka-topics --create --topic filestream-2-distributed --partitions 3 --replication-factor 1 --zookeeper 127.0.0.1:2181
...
# Read the topic data
docker run --rm -it --net=host landoop/fast-data-dev bash
kafka-console-consumer --topic filestream-2-distributed --from-beginning --bootstrap-server 127.0.0.1:9092
...
# create the topic we're going to write to
docker run --rm -it --net=host landoop/fast-data-dev bash
kafka-topics --create --topic distributed-3-twitter --partitions 3 --replication-factor 1 --zookeeper 127.0.0.1:2181
# Start a console consumer on that topic
kafka-console-consumer --topic distributed-3-twitter --bootstrap-server 127.0.0.1:9092
...
# If you don't want any table to track then add it to blacklist(table.blacklist). Similiarly, if you want to track only few tables then add them to whitelist(table.whitelist).
